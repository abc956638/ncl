                                     ...
                                     int *rows, int *cols, int nx, int ny)
{
    ...
    for (right = 0, tmp = 0.0, i = 0; i < nx; i++)
    { /* map cols*/
        left = right;
        tmp += inc;
        right = (int)RINT(tmp);
    ...
    for (right = 0, tmp = 0.0, i = 0; i < ny; i++)
    { /* map rows*/
        left = right;
        tmp += inc;
        right = (int)RINT(tmp);
...
static int cro_CEsc(CROddp *psa, _NGCesc *cesc)
{
    ...
    case NGC_ANTIALIAS: /* settings for cairo's antialiasing */
        ...
        cairo_set_antialias(getContext(psa->wks_id), (aliasConfig->antialias_boolean) ? CAIRO_ANTIALIAS_DEFAULT : CAIRO_ANTIALIAS_NONE);
        break;
...
int cro_FillArea(GKSC *gksc)
{
    ...
    case SOLID_TEXT_FILL:
        ...
        else if (psa->is_vector_type == TRUE)
        { /* pdf, ps, svg, etc. */
...
int cro_SetColorRepresentation(GKSC *gksc)
{
    ...
    {
        psa->ctable = (unsigned int *)realloc(psa->ctable, (index + 1) * sizeof(unsigned int));
        /*
...
int cro_SetViewport(GKSC *gksc)
{
     ...
     *  transformation.
     */
    if ((psa->tsystem.viewport.llx != fptr[0]) || (psa->tsystem.viewport.urx != fptr[1]) || (psa->tsystem.viewport.lly != fptr[2]) || (psa->tsystem.viewport.ury != fptr[3]))
    {
...
int cro_SetWindow(GKSC *gksc)
{
     ...
     *  transformation.
     */
    if ((psa->tsystem.window.llx != fptr[0]) || (psa->tsystem.window.urx != fptr[1]) || (psa->tsystem.window.lly != fptr[2]) || (psa->tsystem.window.ury != fptr[3]))
    {
...
int cro_Text(GKSC *gksc)
{
    ...
    if (kount == 0)
    {
        ...
        font_path = (char *)calloc(strlen(db_path) + strlen("/") + strlen(font_name) + 1, sizeof(char));
     ...
     */
    base_mag = sqrt(psa->attributes.char_base_vec_x * psa->attributes.char_base_vec_x + psa->attributes.char_base_vec_y * psa->attributes.char_base_vec_y);
     ...
     */
    if (psa->attributes.char_space == CHAR_SPACE_DEFAULT && psa->attributes.char_expan == CHAR_EXPAN_DEFAULT && psa->attributes.text_path == TEXT_PATH_DEFAULT)
    {
        ...
        case DOWN_TEXT_PATH:
            ...
            for (i = 0; i < strlen(sptr); i++)
            {
                ...
                if (i == 0)
                {
                     ...
                     */
                    cairo_rel_move_to(context,
                                      (0.5 * textents.width + textents.x_bearing - x_del) - textents.x_advance, 0.);
                }
                else
                {
                    cairo_text_extents(context, single_char, &textents);
...
const char *getFileNameRoot(int wkid, const char *file_name, const char *envVar)
{
     ...
     */
    if (file_name != NULL && strlen(file_name) > 0 && strncmp(file_name, defaultName, strlen(defaultName)) != 0)
    {
...
static void CROinit(CROddp *psa, int *coords)
{
    ...
    curx = *(coords + 2);
    cury = *(coords + 3);
    if ((cllx != -9999) && (clly != -9999) && (curx != -9999) && (cury != -9999))
    {
        ...
        if (knd < nnd)
        {
            ...
            if (fabs(xco) > fabs(yco))
            {
                for (i = knd; i < nnd; ++i)
                {
                    ...
                    ip2 = (ind[i] + 1) % nra;
                    ++lnd;
                    tmp = xco * (points[ip2].x - points[ip1].x) + yco * (points[ip2].y - points[ip1].y);
                    if (fabs(tmp) > smalld)
                    {
                        rst[lnd] = (spi * (points[ip2].y - points[ip1].y) - xco * (points[ip1].x * points[ip2].y - points[ip2].x * points[ip1].y)) / tmp;
                    }
                    else
                    {
                ...
                for (i = knd; i < nnd; ++i)
                {
                    ...
                    if (fabs(tmp) > smalld)
                    {
                        rst[lnd] = (spi * (points[ip2].x - points[ip1].x) + yco *
                                                                                (points[ip1].x * points[ip2].y - points[ip2].x * points[ip1].y)) /
                                   tmp;
                    }
                    else
                    {
